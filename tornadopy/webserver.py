...
class Server(object):
    ...
    def _patch_httpserver(self):
        ...
        class tornadopyHTTPServer(httpserver.HTTPServer):
            def __init__(self, request_callback, xheaders=xhs, **kwargs):
                super(tornadopyHTTPServer, self).__init__(request_callback,
                                                        xheaders=xheaders,
                                                        **kwargs)

        httpserver.HTTPServer = tornadopyHTTPServer
        sys.modules["tornado.httpserver"] = httpserver

    def load_application(self, application=None):
        """

        :type application: tornadopy.application.Application subclass or instance
        :return:
        """
        ...
        if not application:
            ...
            raise ArgumentError('need tornadopy.application.Application instance object or subclass.')
    ...
    def _install_application(self, application):
        ...
        if 'default_handler_class' in tornado_conf and \
            ...
            tornado_conf['default_handler_class'] = import_object('tornadopy.handler.ErrorHandler')
